...
var RedisStore = class extends express_session.Store {
	client;
	prefix;
	scanCount;
	serializer;
	...
	async set(sid, sess, cb) {
		...
		try {
			if (ttl > 0) {
				...
				else await this.client.set(key, val, { expiration: {
					type: "EX",
					value: ttl
				} });
	...
	async length(cb) {
		try {
			return optionalCb(null, (await this.getAllKeys()).length, cb);
		} catch (err) {
			return optionalCb(err, null, cb);
		}
	}
	async ids(cb) {
		...
		try {
			return optionalCb(null, (await this.getAllKeys()).map((k) => k.substring(len)), cb);
		} catch (err) {
			return optionalCb(err, null, cb);
		}
	}
	async all(cb) {
		...
		try {
			...
			return optionalCb(null, (await this.client.mGet(keys)).reduce((acc, raw, idx) => {
				if (!raw) return acc;
	...
	async getAllKeys() {
		...
		for await (let keys of this.scanIterator(pattern, this.scanCount)) for (let key of keys) set.add(key);
		return set.size > 0 ? Array.from(set) : [];
	}
	scanIterator(match, count) {
		let client = this.client;
		if (!("masters" in client)) return client.scanIterator({
			MATCH: match,
			COUNT: count
		});
